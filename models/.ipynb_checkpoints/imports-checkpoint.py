...
def save_nb(nb_name=None, attempts=1, verbose=True, wait=2):
    ...
    if nb_name is None:
        ...
        if is_colab():
            if verbose: print(
                f'cannot save the notebook in Google Colab. Last saved {to_local_time(os.path.getmtime(nb_name))}.')
        else:
...
def all_last_saved(max_elapsed=60):
    ...
    for fp in get_files(folder):
        fp = str(fp)
        fn = fp.split('/')[-1]
        if not fn.endswith(".py") or fn.startswith("_") or fn.startswith(".") or fn in ['imports.py', 'all.py',
                                                                                        'basics.py']:  # add files without a notebook
            continue
...
def py_last_saved(nb_name, max_elapsed=1):
    ...
    if nb_name == "index.ipynb":
        ...
        script_name = str(
            folder / ".".join([str(nb_name).split("_", 1)[1:][0].replace(".ipynb", "").replace(".", "/"), "py"]))
...
def create_scripts(nb_name=None, max_elapsed=60, wait=2):
    ...
    if nb_name is None:
        ...
        output = py_last_saved(nb_name=nb_name, max_elapsed=max_elapsed)
    beep(output)
    return output
